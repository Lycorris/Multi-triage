...
def dataset_preprocess_multi_repo(_paths, _code_format, _ckpt):
    ...
    D_tokens2id = {D_token: D_id for D_id, D_token in enumerate(D_ids2token)}
    B_tokens2id = {B_token: B_id for B_id, B_token in enumerate(B_ids2token)}

    # reconvert datasets output
    datasets = [label_vectorize_by_dict(dataset, (D_tokens2id, B_tokens2id))
